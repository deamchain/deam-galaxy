...
// It offers only methods that galaxyte on public data that is freely available to anyone.
...
// PublicTxPoolAPI offers and API for the transaction pool. It only galaxytes on data that is non confidential.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f DEAM) exceeds the configured cap (%.2f DEAM)", feeFloat, cap)
	}
	return nil
}
